...
const App = () => {
  ...
  useEffect(() => {
    ...
    const id = localStorage.getItem("userId") ?? `user_${Math.floor(Math.random() * 1000000)}`;
    localStorage.setItem("userId", id);

    ws.current = new WebSocket(`ws://localhost:3001/?userId=${id}`);
    ws.current.addEventListener("open", () => {
      if(ws.current) {
        console.log("Sending data. We're open!")
        ws.current.send(JSON.stringify({ type: "username:set", userId: id, username: `Player_${id.substring(5, 10)}` }));
      }
    });
