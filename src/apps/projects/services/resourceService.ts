...
import { type AssetCategory, type AssetNode, type ProjectDescriptor, type ProjectManifest, type ResourceNode } from "../types/resource";
...
const projectsRoot = path.resolve(contentRoot, "projects");
...
const sanitizeAssetPaths = (values: unknown): string[] => {
...
export const resourceService = {
  ...
  async listProjects(): Promise<ProjectDescriptor[]> {
    const folderProjects = await this.listFolderProjects();
    const byId = new Map<string, ProjectDescriptor>();
    folderProjects.forEach((project) => byId.set(project.id, project));
  ...
  async getProjectById(projectId: string): Promise<ProjectDescriptor | null> {
    ...
    const found = projects.find((project) => project.id === normalizedId);
    return found ?? null;
  },
  ...
  async listFolderProjects(): Promise<ProjectDescriptor[]> {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        const id = normalizeProjectId(
          typeof manifest.id === "string" && manifest.id.trim().length > 0 ? manifest.id : entry.name
        );
        if (!id) {
          continue;
        }
        ...
        projects.push({
          ...
          title,
          startupLevel,
          manifestPath: path.relative(contentRoot, manifestPath).replace(/\\/g, "/"),
          projectPath: path.relative(contentRoot, projectDir).replace(/\\/g, "/"),
          assetRoots: sanitizeAssetPaths(manifest.assetRoots),
          assetFiles: sanitizeAssetPaths(manifest.assetFiles),
        });
      } catch (error) {
        console.warn(`Failed to parse project manifest ${manifestPath}`, error);
      }
    }
  ...
  async listLegacyProjects(): Promise<ProjectDescriptor[]> {
    if (!(await this.pathExists(projectsRoot))) {
      return [];
    }
    ...
    for (const entry of entries) {
      ...
      const manifestPath = path.join(projectsRoot, entry.name);
      try {
        const raw = await fs.readFile(manifestPath, "utf-8");
        const manifest = JSON.parse(raw) as ProjectManifest;
        const startupLevel = typeof manifest.startupLevel === "string" ? manifest.startupLevel.trim() : "";
        ...
        const inferredName = entry.name.replace(/\.project\.json$/i, "");
        const id = normalizeProjectId(
          typeof manifest.id === "string" && manifest.id.trim().length > 0 ? manifest.id : inferredName
        );
        if (!id) {
          continue;
        }
        ...
        console.warn(`Failed to parse legacy project manifest ${manifestPath}`, error);
      }
    }
